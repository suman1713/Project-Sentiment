...
var oauth = {
  callback: `${process.env.TWITTER_CALLBACK_HOST}/api/oauth/confirm`,
...
module.exports.getAccessToken = function getAccessToken(verify, cb) {
  request.post({ url: 'https://api.twitter.com/oauth/access_token', oauth: Object.assign({}, oauth, verify) },
  (err, res, body) => {
    if (err) console.error(err);
    var reqData = qs.parse(body);
    cb(reqData);
